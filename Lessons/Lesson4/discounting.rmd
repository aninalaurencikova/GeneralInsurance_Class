...
```{r}
#paid data, house business, large claim size
Paid_HH_lrg <- dt_PaidCase %>% 
  filter(Business == "House" & ClaimSize == "Large" & dataset_type == "PAID")
Paid_HH_lrg <- Paid_HH_lrg %>% as.triangle(Paid_HH_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Paid_HH_lrg)
plot(predict(chainladder(Paid_HH_lrg)))
ata(Paid_HH_lrg)

#paid data, 3rd party business, small claim size
Paid_3rd_sml <- dt_PaidCase %>% 
  filter(Business == "3rd Party" & ClaimSize == "Small" & dataset_type == "PAID")
Paid_3rd_sml <- Paid_3rd_sml %>% as.triangle(Paid_3rd_sml, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Paid_3rd_sml)
plot(predict(chainladder(Paid_3rd_sml)))
ata(Paid_3rd_sml)

#paid data, 3rd party business, large claim size
Paid_3rd_lrg <- dt_PaidCase %>% 
  filter(Business == "3rd Party" & ClaimSize == "Large" & dataset_type == "PAID")
Paid_3rd_lrg <- Paid_3rd_lrg %>% as.triangle(Paid_3rd_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Paid_3rd_lrg)
plot(predict(chainladder(Paid_3rd_lrg)))
ata(Paid_3rd_lrg)

#Najvacsie rozdiely su medzi House a 3rd party business pri Small Claim Size. Naklady pri 3rd party stale rastu, avsak pri House rastu v prvych rokoch a potom zostanu takmer konstatne.
```
...
```{r}
#house business, small claim size, paid+case
Incur_HH_sml <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Small") 
Incur_HH_sml <- Incur_HH_sml %>% as.triangle(Incur_HH_sml, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Incur_HH_sml)
plot(predict(chainladder(Incur_HH_sml)))
ata(Incur_HH_sml)

#house business, large claim size, paid+case
Incur_HH_lrg <- dt_PaidCase %>% filter(Business == "House" & ClaimSize == "Large") 
Incur_HH_lrg <- Incur_HH_lrg %>% as.triangle(Incur_HH_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Incur_HH_lrg)
plot(predict(chainladder(Incur_HH_lrg)))
ata(Incur_HH_lrg)

#3rd party business, small claim size, paid+case
Incur_3rd_sml <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Small") 
Incur_3rd_sml <- Incur_3rd_sml %>% as.triangle(Incur_3rd_sml, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Incur_3rd_sml)
plot(predict(chainladder(Incur_3rd_sml)))
ata(Incur_3rd_sml)

#3rd party business, large claim size, paid+case
Incur_3rd_lrg <- dt_PaidCase %>% filter(Business == "3rd Party" & ClaimSize == "Large") 
Incur_3rd_lrg <- Incur_3rd_lrg %>% as.triangle(Incur_3rd_lrg, origin = "ay", dev = "dy", value = "SumOfamount")
plot(Incur_3rd_lrg)
plot(predict(chainladder(Incur_3rd_lrg)))
ata(Incur_3rd_lrg)
```
...
```{r}
#House business - cca 4  roky trva, pokial sa losses zastabilizuju a dalej sa radikalne nemenia. Pri 3rd party to trva dlhsie.

#Pri House business - ak sa nieco stane, tak sa to zaplati a predpoklada sa, ze s tym nebudu spojene dalsie vyznamne naklady (preto sa naklady "zastabiluzuju"). Pri 3rd party business moze nahrada skody trvat nepomerne dlhsie.
```
...
#netusim co dalej s tymto cvicenim
```
